...
!   ! overload LPHYLIN
    ...
    CALL INITIALIZE_PARAMETER_DATATYPES(OUTPUT_YDMCST, OUTPUT_YDOETHF, OUTPUT_YDECLD, &
                                        OUTPUT_YDECLDP, OUTPUT_YDEPHLI, OUTPUT_YDPHNC )
     print *, 'Finishing driver init. RG is',OUTPUT_YDMCST%RG
  CONTAINS 
SUBROUTINE INITIALIZE_PARAMETER_DATATYPES(YDOMCST, YDOETHF, YDECLD, YDECLDP, YDEPHLI, YDPHNC )
USE YOMCST   , ONLY : RETV, RG  ,RCPD     ,&
 & RLVTT    ,RLSTT    ,RLMLT    ,RTT     ,RD, &
   RA, RPI, RV
USE YOETHF   , ONLY : R2ES     ,R3LES    ,R3IES    ,R4LES    ,&
