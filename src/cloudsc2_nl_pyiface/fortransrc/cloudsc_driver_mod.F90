...
! extracted from the topmost dwarf_cloudsc.F90 experiment driver
  ...
  SUBROUTINE CLOUDSC_DRIVER_INIT( &
     ...
     & OUTPUT_YDECLDP, OUTPUT_YDEPHLI, OUTPUT_YDPHNC, & 
       NUMOMP, NPROMA, NLEV, NCLV,  NGPTOT, NGPBLKS, & 
     &  NGPTOTG, &
       PTSPHY, &
     & OUTPUT_PT, OUTPUT_PQ, &
  ...
  USE YOPHNC   , ONLY : YRPHNC
    ...
    TYPE(TOMCST)   ,    INTENT(OUT) :: OUTPUT_YDMCST 
    TYPE(TOETHF)   ,    INTENT(OUT) :: OUTPUT_YDOETHF 
    TYPE(TECLD)    ,    INTENT(INOUT) :: OUTPUT_YDECLD 
    TYPE(TECLDP)   ,    INTENT(OUT) :: OUTPUT_YDECLDP
    TYPE(TEPHLI)   ,    INTENT(OUT) :: OUTPUT_YDEPHLI
    TYPE(TPHNC)    ,    INTENT(OUT) :: OUTPUT_YDPHNC
...
!   YREPHLI%LPHYLIN=.true.
    ...
    OUTPUT_YDECLD%CETA(:)=YRECLD%CETA(:)
    OUTPUT_YDMCST%RG   =RG
    OUTPUT_YDMCST%RG   =RD
  ...
  SUBROUTINE CLOUDSC_DRIVER_NO_DERV_TPES( &
     ...
     & NUMOMP, NPROMA, NLEV, NCLV, NCLDQL, NCLDQI, NGPTOT, NGPBLKS,  NGPTOTG, PTSPHY, &
     & PT, PQ, &
