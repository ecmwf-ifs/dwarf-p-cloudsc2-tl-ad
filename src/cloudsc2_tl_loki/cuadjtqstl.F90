...
!    *ZPP5*         PRESSURE                                    (Trajectory)
!    *ZPP*          PRESSURE                                        PA

!     UPDATED PARAMETERS (REAL):

!    *ZTP15*          TEMPERATURE                                (Trajectory) 
!    *ZQP15*          SPECIFIC HUMIDITY                          (Trajectory) 
!    *ZTP1*           TEMPERATURE                                     K
!    *ZQP1*           SPECIFIC HUMIDITY                             KG/KG
...
REAL(KIND=JPRB)   ,INTENT(IN)    :: ZPP5(KLON) 
REAL(KIND=JPRB)   ,INTENT(INOUT) :: ZTP15(KLON,KLEV) 
REAL(KIND=JPRB)   ,INTENT(INOUT) :: ZQP15(KLON,KLEV) 
REAL(KIND=JPRB)   ,INTENT(IN)    :: ZPP(KLON) 
REAL(KIND=JPRB)   ,INTENT(INOUT) :: ZTP1(KLON,KLEV) 
REAL(KIND=JPRB)   ,INTENT(INOUT) :: ZQP1(KLON,KLEV) 
...
ENDDO
  ...
  DO JL=KIDIA,KFDIA
    ...
    ZCOND1 (JL)=(ZQP1(JL,JK)-ZQSAT (JL))/&
     & (1.0_JPRB+ZQSAT5(JL)*ZCOR5(JL)*Z2S5(JL)) -&
     & (ZQP15(JL,JK)-ZQSAT5(JL))*&
     & (ZQSAT (JL)*ZCOR5(JL)*Z2S5(JL)   +&
     & ZQSAT5(JL)*ZCOR (JL)*Z2S5(JL)   +&
     & ZQSAT5(JL)*ZCOR5(JL)*Z2S (JL))/&
     & (1.0_JPRB+ZQSAT5(JL)*ZCOR5(JL)*Z2S5(JL))**2  
    ZCOND15(JL)=(ZQP15(JL,JK)-ZQSAT5(JL))/(1.0_JPRB+ZQSAT5(JL)*ZCOR5(JL)*Z2S5(JL))
    ZTP1(JL,JK)=ZTP1 (JL,JK)+ZALDCP(JL)*ZCOND1 (JL)
    ZTP15(JL,JK)=ZTP15(JL,JK)+ZALDCP(JL)*ZCOND15(JL)
    ZQP1(JL,JK)=ZQP1(JL,JK)-ZCOND1 (JL)
    ZQP15(JL,JK)=ZQP15(JL,JK)-ZCOND15(JL)
  ENDDO
