...
REAL(KIND=JPRB) :: ZRFLN5(KLON,KLEV), ZSFLN5(KLON,KLEV)
REAL(KIND=JPRB) :: ZGDP5(KLON,KLEV)
REAL(KIND=JPRB) :: ZDQDT5(KLON,KLEV), ZDTDT5(KLON,KLEV), ZDLDT5(KLON,KLEV), ZDIDT5(KLON,KLEV)
REAL(KIND=JPRB) :: ZQCRIT5(KLON,KLEV)
REAL(KIND=JPRB) :: ZCOVPTOT5(KLON,KLEV)
REAL(KIND=JPRB) :: ZCOVPCLR5(KLON,KLEV)
REAL(KIND=JPRB) :: ZDQSDTEMP5(KLON,KLEV), ZCORQS5(KLON,KLEV)
REAL(KIND=JPRB) :: ZDTGDP5(KLON,KLEV)
REAL(KIND=JPRB) :: ZQOLD5(KLON,KLEV)
REAL(KIND=JPRB) :: ZDQ5(KLON,KLEV)
REAL(KIND=JPRB) :: ZQLIM5(KLON,KLEV)
REAL(KIND=JPRB) :: ZRFREEZE5(KLON,KLEV),ZPP5(KLON,KLEV)
REAL(KIND=JPRB) :: ZCONDL5(KLON,KLEV), ZCONDI5(KLON,KLEV)
REAL(KIND=JPRB) :: ZEVAPR5(KLON,KLEV), ZEVAPS5(KLON,KLEV)
REAL(KIND=JPRB) :: ZQSAT5(KLON,KLEV)
REAL(KIND=JPRB) :: ZFWAT5(KLON,KLEV)
...
REAL(KIND=JPRB) :: ZRFL(KLON,KLEV+1), ZSFL(KLON,KLEV+1)
REAL(KIND=JPRB) :: ZDP(KLON,KLEV)
REAL(KIND=JPRB) :: ZLSDCP(KLON,KLEV), ZLFDCP(KLON,KLEV), ZLVDCP(KLON,KLEV)
REAL(KIND=JPRB) :: ZRFLN(KLON,KLEV), ZSFLN(KLON,KLEV)
REAL(KIND=JPRB) :: ZGDP(KLON,KLEV)
REAL(KIND=JPRB) :: ZDLDT(KLON,KLEV), ZDIDT(KLON,KLEV)
REAL(KIND=JPRB) :: ZDTDT(KLON,KLEV), ZDQDT(KLON,KLEV)
REAL(KIND=JPRB) :: ZQCRIT(KLON,KLEV)
REAL(KIND=JPRB) :: ZCOVPTOT(KLON,KLEV)
REAL(KIND=JPRB) :: ZCOVPCLR(KLON,KLEV)
REAL(KIND=JPRB) :: ZDQSDTEMP(KLON,KLEV), ZCORQS(KLON,KLEV) 
REAL(KIND=JPRB) :: ZDTGDP(KLON,KLEV)
REAL(KIND=JPRB) :: ZQOLD(KLON,KLEV)
REAL(KIND=JPRB) :: ZDQ(KLON,KLEV)
REAL(KIND=JPRB) :: ZQLIM(KLON,KLEV)
REAL(KIND=JPRB) :: ZRFREEZE(KLON,KLEV), ZPP(KLON, KLEV)
...
REAL(KIND=JPRB) :: ZFWAT(KLON,KLEV)

REAL(KIND=JPRB) :: ZSCALMT(KLEV)
...
REAL(KIND=JPRB) :: ZOEALFAW5, ZFAC5, ZFACW5, ZFACI5, ZESDP5
...
REAL(KIND=JPRB) :: ZQP5(KLON,KLEV), ZCOR5(KLON,KLEV)
REAL(KIND=JPRB) :: ZQP(KLON,KLEV), ZCOR(KLON,KLEV)
REAL(KIND=JPRB) :: ZCKCODTL, ZCKCODTI, ZCONS2, ZCONS3, ZMELTP2
REAL(KIND=JPRB) ::  ZQMAX, ZQTMST
REAL(KIND=JPRB) ::  Z3ES(KLON,KLEV),             Z4ES(KLON,KLEV), &
 & Z5ALCP(KLON,KLEV),           ZALDCP(KLON,KLEV)
REAL(KIND=JPRB) ::  ZTARG5(KLON,KLEV), ZFOEEW5(KLON,KLEV)
REAL(KIND=JPRB) ::  Z2S5(KLON,KLEV), ZCOND15(KLON,KLEV)
REAL(KIND=JPRB) :: ZTARG(KLON,KLEV),  ZFOEEW(KLON,KLEV)
REAL(KIND=JPRB) :: Z2S(KLON,KLEV)
REAL(KIND=JPRB) :: ZCOND1(KLON,KLEV)
REAL(KIND=JPRB) :: ZRH1,ZRH2,ZRH3,ZETA2,ZDETA1,ZDETA2 
REAL(KIND=JPRB) :: ZTRPAUS(KLON,KLEV)
...
ASSOCIATE( RETV=>YDCST%RETV, RG=>YDCST%RG, RCPD=>YDCST%RCPD, &
RLVTT=>YDCST%RLVTT, RLSTT=>YDCST%RLSTT, RLMLT=>YDCST%RLMLT, &
RTT=>YDCST%RTT, RD=>YDCST%RD, R2ES=>YDTHF%R2ES, R3LES=>YDTHF%R3LES, &
R3IES=>YDTHF%R3IES, R4LES=>YDTHF%R4LES, R4IES=>YDTHF%R4IES, &
R5LES=>YDTHF%R5LES, R5IES=>YDTHF%R5IES, RTICE=>YDTHF%RTICE, &
RCLCRIT=>YCLDP%RCLCRIT, RKCONV=>YCLDP%RKCONV, RLMIN=>YCLDP%RLMIN, &
RPECONS=>YCLDP%RPECONS, RLPTRC=>YPHLI%RLPTRC, LREGCL=>YNCL%LREGCL, LEVAPLS2=>YHNC%LEVAPLS2, &
RVTMP2=>YDTHF%RVTMP2, &
...
!       Set to zero precipitation fluxes at the top
  ...
  DO JL=KIDIA,KFDIA
    ZRFL (JL,JK) = 0.0_JPRB
    ZRFL5(JL,JK) = 0.0_JPRB
    ZSFL (JL,JK) = 0.0_JPRB
    ZSFL5(JL,JK) = 0.0_JPRB
    PFPLSL (JL,1) = 0.0_JPRB
    PFPLSL5(JL,1) = 0.0_JPRB
    PFPLSN (JL,1) = 0.0_JPRB
    PFPLSN5(JL,1) = 0.0_JPRB
    ZCOVPTOT(JL,JK) = 0.0_JPRB
    ZCOVPTOT5(JL,JK) = 0.0_JPRB
    ZCOVPCLR(JL,JK) = 0.0_JPRB
    ZCOVPCLR5(JL,JK) = 0.0_JPRB
  ENDDO
  
  DO JL=KIDIA,KFDIA
    ZTRPAUS(JL,JK)=0.1_JPRB
  ENDDO
ENDDO
...
!-----------------------------------
    ...
    IF (ZTP15(JL,JK) < RTT) THEN
      ZFWAT (JL,JK) = ZOEALFAW
      ZFWAT5(JL,JK) = ZOEALFAW5
      Z3ES(JL,JK)=R3IES
      Z4ES(JL,JK)=R4IES
    ELSE
      ...
      Z4ES(JL,JK)=R4LES
    ENDIF
    ZFOEEW5(JL,JK) = R2ES*EXP(Z3ES(JL,JK)*(ZTP15(JL,JK)-RTT)/(ZTP15(JL,JK)-Z4ES(JL,JK)))
    ZFOEEW (JL,JK) = Z3ES(JL,JK)*(RTT-Z4ES(JL,JK))*ZTP1(JL,JK)*ZFOEEW5(JL,JK) &
              & / (ZTP15(JL,JK)-Z4ES(JL,JK))**2  
    ZESDP  = ZFOEEW (JL,JK)/PAPP15(JL,JK) - PAPP1(JL,JK) &
         & * ZFOEEW5(JL,JK)/PAPP15(JL,JK)**2  
    ZESDP5 = ZFOEEW5(JL,JK)/PAPP15(JL,JK)
    ...
    ZFAC  = ZFWAT5(JL,JK)*ZFACW+ZFACW5*ZFWAT (JL,JK)+(1.0_JPRB-ZFWAT5(JL,JK))*ZFACI &
     & - ZFACI5*ZFWAT (JL,JK)  
    ZFAC5 = ZFWAT5(JL,JK)*ZFACW5+(1.0_JPRB-ZFWAT5(JL,JK))*ZFACI5
    ZCOR(JL,JK)  = RETV*ZESDP/(1.0_JPRB-RETV*ZESDP5)**2
    ZCOR5(JL,JK) = 1.0_JPRB/(1.0_JPRB-RETV*ZESDP5)
    ZDQSDTEMP(JL,JK) = ZFAC5*ZCOR5(JL,JK)*PQS(JL,JK)+ZFAC5*PQS5(JL,JK)*ZCOR(JL,JK) &
     & + ZCOR5(JL,JK)*PQS5(JL,JK)*ZFAC  
    ZDQSDTEMP5(JL,JK) = ZFAC5*ZCOR5(JL,JK)*PQS5(JL,JK)
    ZCORQS(JL,JK) = ZCONS3*ZDQSDTEMP(JL,JK)
    ZCORQS5(JL,JK) = 1.0_JPRB+ZCONS3*ZDQSDTEMP5(JL,JK)

! use clipped state

    IF (ZQP15(JL,JK)>PQS5(JL,JK)) THEN
      ZQLIM (JL,JK) = PQS (JL,JK)
      ZQLIM5(JL,JK) = PQS5(JL,JK)
    ELSE
      ZQLIM (JL,JK) = ZQP1(JL,JK)
      ZQLIM5(JL,JK) = ZQP15(JL,JK)
    ENDIF
...
! Allow ice supersaturation at cold temperatures
    ...
    ELSE
      ...
      ZSUPSAT  = 0.0_JPRB
    ENDIF
    ZQSAT5(JL,JK)=PQS5(JL,JK)*ZSUPSAT5
    ZQSAT (JL,JK)=PQS (JL,JK)*ZSUPSAT5 + PQS5(JL,JK)*ZSUPSAT

    ZQCRIT5(JL,JK)=ZCRH2*ZQSAT5(JL,JK)
    ZQCRIT (JL,JK)=ZCRH2*ZQSAT (JL,JK)
  ENDDO
...
! Simple UNIFORM distribution of total water from Letreut & Li (90)
  ...
  DO JL=KIDIA,KFDIA
    ...
    IF (ZQT5 <= ZQCRIT5(JL,JK)) THEN
      PCLC (JL,JK) = 0.0_JPRB
      PCLC5(JL,JK) = 0.0_JPRB
      ZQC (JL,JK) = 0.0_JPRB
      ZQC5(JL,JK) = 0.0_JPRB
    ELSEIF (ZQT5 >= ZQSAT5(JL,JK)) THEN
      PCLC (JL,JK) = 0.0_JPRB
      PCLC5(JL,JK) = 1.0_JPRB
      ZQC (JL,JK) = (1.0_JPRB-ZSCALMT(JK))*(ZQSAT (JL,JK)-ZQCRIT (JL,JK))
      ZQC5(JL,JK) = (1.0_JPRB-ZSCALMT(JK))*(ZQSAT5(JL,JK)-ZQCRIT5(JL,JK))
    ELSE
      ZQPD  = ZQSAT (JL,JK)-ZQT
      ZQPD5 = ZQSAT5(JL,JK)-ZQT5
      ZQCD  = ZQSAT (JL,JK)-ZQCRIT (JL,JK)
      ZQCD5 = ZQSAT5(JL,JK)-ZQCRIT5(JL,JK)

      ZSQRT5 = SQRT(ZQPD5/(ZQCD5-ZSCALMT(JK)*(ZQT5-ZQCRIT5(JL,JK))))  
      PCLC5(JL,JK) = 1.0_JPRB-ZSQRT5
      PCLC (JL,JK) = -(0.5_JPRB/ZSQRT5) &
       & * (ZQPD *(ZQCD5-ZSCALMT(JK)*(ZQT5-ZQCRIT5(JL,JK))) &
       & -  ZQPD5*(ZQCD -ZSCALMT(JK)*(ZQT -ZQCRIT (JL,JK)))) &
       & / (ZQCD5-ZSCALMT(JK)*(ZQT5-ZQCRIT5(JL,JK)))**2  
...
! Add convective component
  ...
  DO JL=KIDIA,KFDIA
    ZGDP (JL,JK) = - RG*(PAPHP1(JL,JK+1)-PAPHP1(JL,JK)) &
     & / (PAPHP15(JL,JK+1)-PAPHP15(JL,JK))**2  
    ZGDP5(JL,JK) = RG/(PAPHP15(JL,JK+1)-PAPHP15(JL,JK))

    ZLUDE (JL,JK) = PTSPHY*ZGDP5(JL,JK)*PLUDE(JL,JK) &
     & + PTSPHY*PLUDE5(JL,JK)*ZGDP(JL,JK)  
...
! Add compensating subsidence component
  ...
  DO JL=KIDIA,KFDIA
    ...
    ZFAC2=1.0_JPRB/(PAPP15(JL,JK)-RETV*ZFOEEW5(JL,JK))
    ZRODQSDP = (-ZRHO *PQS5(JL,JK) - ZRHO5*PQS (JL,JK) + ZRHO5*PQS5(JL,JK) &
           & *(PAPP1 (JL,JK)-RETV*ZFOEEW (JL,JK))*ZFAC2)*ZFAC2
    ZRODQSDP5 = -ZRHO5*PQS5(JL,JK)*ZFAC2

    ZLDCP  = ZFWAT (JL,JK)*ZLVDCP5(JL,JK) + ZFWAT5(JL,JK)*ZLVDCP (JL,JK) &
         & + (1.0_JPRB-ZFWAT5(JL,JK))*ZLSDCP (JL,JK) - ZFWAT (JL,JK)*ZLSDCP5(JL,JK)
    ZLDCP5 = ZFWAT5(JL,JK)*ZLVDCP5(JL,JK) + (1.0_JPRB-ZFWAT5(JL,JK))*ZLSDCP5(JL,JK)

    ZFAC3=1.0_JPRB/(1.0_JPRB + ZLDCP5*ZDQSDTEMP5(JL,JK))
    DTDZMO5 = RG*(1.0_JPRB/RCPD - ZLDCP5*ZRODQSDP5)*ZFAC3
    DTDZMO  = -(RG*(ZLDCP *ZRODQSDP5 + ZLDCP5*ZRODQSDP )&
          & + DTDZMO5*(ZLDCP5*ZDQSDTEMP(JL,JK) + ZLDCP *ZDQSDTEMP5(JL,JK)))*ZFAC3

    ZDQSDZ  = ZDQSDTEMP5(JL,JK)*DTDZMO  + ZDQSDTEMP(JL,JK)*DTDZMO5 - RG*ZRODQSDP
    ZDQSDZ5 = ZDQSDTEMP5(JL,JK)*DTDZMO5 - RG*ZRODQSDP5
...
! New cloud liquid/ice contents and condensation rates (liquid/ice)
  ...
  DO JL=KIDIA,KFDIA
    ZQLWC (JL,JK)=ZQC (JL,JK)*ZFWAT5(JL,JK)+ZQC5(JL,JK)*ZFWAT (JL,JK)
    ZQLWC5(JL,JK)=ZQC5(JL,JK)*ZFWAT5(JL,JK)

    ZQIWC (JL,JK)=ZQC (JL,JK)*(1.0_JPRB-ZFWAT5(JL,JK))-ZQC5(JL,JK)*ZFWAT (JL,JK)
    ZQIWC5(JL,JK)=ZQC5(JL,JK)*(1.0_JPRB-ZFWAT5(JL,JK))
...
! Simple form based on Maximum Overlap.
  ...
  DO JL=KIDIA,KFDIA  
    IF (PCLC5(JL,JK) > ZCOVPTOT5(JL,JK)) THEN ! total rain frac
      ZCOVPTOT(JL,JK) = PCLC (JL,JK)
      ZCOVPTOT5(JL,JK) = PCLC5(JL,JK)
    ENDIF
    ZCOVPCLR(JL,JK) = ZCOVPTOT(JL,JK)-PCLC(JL,JK)
    ZCOVPCLR5(JL,JK) = ZCOVPTOT5(JL,JK)-PCLC5(JL,JK)
    IF (ZCOVPCLR5(JL,JK) < 0.0_JPRB) THEN
      ZCOVPCLR(JL,JK) = 0.0_JPRB
      ZCOVPCLR5(JL,JK) = 0.0_JPRB
    ENDIF
  ENDDO
  ...
  DO JL=KIDIA,KFDIA
    IF (ZSFL5(JL,JK) /= 0.0_JPRB) THEN
      ...
      ELSE
        ...
        ZSNMLT5 = ZZ2S5
      ENDIF
      ZRFLN (JL,JK) = ZRFL (JL,JK)+ZSNMLT
      ZRFLN5(JL,JK) = ZRFL5(JL,JK)+ZSNMLT5
      ZSFLN (JL,JK) = ZSFL (JL,JK)-ZSNMLT
      ZSFLN5(JL,JK) = ZSFL5(JL,JK)-ZSNMLT5
      ZTP1 (JL,JK) = ZTP1 (JL,JK)-(ZSNMLT*ZCONS5-ZCONS*ZSNMLT5)/ZCONS5**2
      ZTP15(JL,JK) = ZTP15(JL,JK)-ZSNMLT5/ZCONS5
    ELSE
      ZRFLN (JL,JK) = ZRFL (JL,JK)
      ZRFLN5(JL,JK) = ZRFL5(JL,JK)
      ZSFLN (JL,JK) = ZSFL (JL,JK)
...
! Rain fraction (different from cloud liquid water fraction!)
    ...
    ZRFLN (JL,JK) = ZRFLN (JL,JK)+ZRN
    ZRFLN5(JL,JK) = ZRFLN5(JL,JK)+ZRN5
    ZSFLN (JL,JK) = ZSFLN (JL,JK)+ZSN
    ZSFLN5(JL,JK) = ZSFLN5(JL,JK)+ZSN5

!   Precip evaporation

    ZPRTOT  = ZRFLN (JL,JK)+ZSFLN (JL,JK)
    ZPRTOT5 = ZRFLN5(JL,JK)+ZSFLN5(JL,JK)
    LLO2=ZPRTOT5>ZEPS2 .AND. ZCOVPCLR5(JL,JK)>ZEPS2 .AND. (LEVAPLS2 .OR. LDRAIN1D)
    IF (LLO2) THEN
      ZPRECLR  = (ZPRTOT5*ZCOVPCLR(JL,JK)+ZCOVPCLR5(JL,JK)*ZPRTOT) &
       & / ZCOVPTOT5(JL,JK) &
       & - ZPRTOT5*ZCOVPCLR5(JL,JK)*ZCOVPTOT(JL,JK) &
       & / ZCOVPTOT5(JL,JK)**2  
      ZPRECLR5 = ZPRTOT5*ZCOVPCLR5(JL,JK)/ZCOVPTOT5(JL,JK)

!     This is the humidity in the moistest zcovpclr region

      ZQE  = PQS(JL,JK) - ((PQS5(JL,JK)-ZQLIM5(JL,JK))*ZCOVPCLR(JL,JK) &
       & + ZCOVPCLR5(JL,JK)*PQS(JL,JK)-ZCOVPCLR5(JL,JK)*ZQLIM(JL,JK)) &
       & / (1.0_JPRB-PCLC5(JL,JK))**2 &
       & - 2.0_JPRB*(PQS5(JL,JK)-ZQLIM5(JL,JK))*ZCOVPCLR5(JL,JK) &
       & * PCLC(JL,JK)/(1.0_JPRB-PCLC5(JL,JK))**3  
      ZQE5 = PQS5(JL,JK)-(PQS5(JL,JK)-ZQLIM5(JL,JK))*ZCOVPCLR5(JL,JK)&
       & / (1.0_JPRB-PCLC5(JL,JK))**2  

      ZBETA  = 0.5777_JPRB*(RG*RPECONS/5.09E-3_JPRB) &
       & * (5.09E-3_JPRB*ZCOVPCLR5(JL,JK)/(ZPRECLR5 &
       ...
       & * ZCOVPCLR(JL,JK) / ZCOVPCLR5(JL,JK)**2)  
...
!     implicit solution:
      ...
      ZB  = PTSPHY*((PQS5(JL,JK)-ZQE5)*ZBETA+ZBETA5*PQS(JL,JK) &
       & - ZBETA5*ZQE)/(1.0_JPRB+ZBETA5*PTSPHY*ZCORQS5(JL,JK)) &
       & - (PTSPHY**2)*ZBETA5*(PQS5(JL,JK)-ZQE5) &
       & * (ZBETA5*ZCORQS(JL,JK)+ZCORQS5(JL,JK)*ZBETA) &
       & / (1.0_JPRB+ZBETA5*PTSPHY*ZCORQS5(JL,JK))**2  
      ZB5 = PTSPHY*ZBETA5*(PQS5(JL,JK)-ZQE5) &
       & / (1.0_JPRB+ZBETA5*PTSPHY*ZCORQS5(JL,JK))  

!     exact solution:

!     ZEXP5 = EXP(-ZBETA5*ZCORQS5(JL,JK)*PTSPHY)
!     ZB  = (ZEXP5*(-ZCORQS5(JL,JK)*(PQS(JL,JK)-ZQE) &
!      &  + (PQS5(JL,JK)-ZQE5) &
!      &  * (PTSPHY*ZCORQS5(JL,JK)*ZBETA5*ZCORQS(JL,JK) &
!      &  + PTSPHY*ZCORQS5(JL,JK)*ZCORQS5(JL,JK)*ZBETA + ZCORQS(JL,JK))) &
!      &  + ZCORQS5(JL,JK)*(PQS(JL,JK)-ZQE) &
!      &  - (PQS5(JL,JK)-ZQE5)*ZCORQS(JL,JK)) &
!      &  / ZCORQS5(JL,JK)**2
!     ZB5 =(PQS5(JL,JK)-ZQE5)*(_ONE_-ZEXP5)/ZCORQS5(JL,JK)

      ZDTGDP (JL,JK) = - PTSPHY*RG*(PAPHP1(JL,JK+1)-PAPHP1(JL,JK)) &
       & / (PAPHP15(JL,JK+1)-PAPHP15(JL,JK))**2  
      ZDTGDP5(JL,JK) =PTSPHY*RG/(PAPHP15(JL,JK+1)-PAPHP15(JL,JK))
      ZDPR  = (ZCOVPCLR5(JL,JK)*ZB+ZB5*ZCOVPCLR(JL,JK)) &
       & / ZDTGDP5(JL,JK) &
       & - ZCOVPCLR5(JL,JK)*ZB5*ZDTGDP(JL,JK)/ZDTGDP5(JL,JK)**2  
      ZDPR5 = ZCOVPCLR5(JL,JK)*ZB5/ZDTGDP5(JL,JK)
      ...
      IF (ZPRECLR5 <= 0.0_JPRB) THEN !reset
        ...
        ZCOVPTOT5(JL,JK) = PCLC5(JL,JK)
      ENDIF
      PCOVPTOT (JL,JK) = ZCOVPTOT(JL,JK)
      PCOVPTOT5(JL,JK) = ZCOVPTOT5(JL,JK)

! warm proportion
      ZEVAPR (JL,JK) = (ZDPR5*ZRFLN(JL,JK)+ZRFLN5(JL,JK)*ZDPR)/ZPRTOT5 &
                   & - ZDPR5*ZRFLN5(JL,JK)*ZPRTOT/ZPRTOT5**2  
      ZEVAPR5(JL,JK) = ZDPR5*ZRFLN5(JL,JK)/ZPRTOT5     
      ZRFLN (JL,JK) = ZRFLN (JL,JK)-ZEVAPR (JL,JK)
      ZRFLN5(JL,JK) = ZRFLN5(JL,JK)-ZEVAPR5(JL,JK) 
     
! ice proportion
      ZEVAPS (JL,JK) = (ZDPR5*ZSFLN(JL,JK)+ZSFLN5(JL,JK)*ZDPR)/ZPRTOT5 &
                   & - ZDPR5*ZSFLN5(JL,JK)*ZPRTOT/ZPRTOT5**2  
      ZEVAPS5(JL,JK) = ZDPR5*ZSFLN5(JL,JK)/ZPRTOT5        
      ZSFLN (JL,JK) = ZSFLN (JL,JK)-ZEVAPS (JL,JK)
      ZSFLN5(JL,JK) = ZSFLN5(JL,JK)-ZEVAPS5(JL,JK)
    ENDIF
...
!   Incrementation of T and Q and fluxes' swap
  ...
  DO JL=KIDIA,KFDIA
    ZDQDT(JL,JK)=-(ZCONDL (JL,JK)+ZCONDI (JL,JK)) &
            & +(PLUDE (JL,JK)+ZEVAPR (JL,JK)+ZEVAPS (JL,JK))*ZGDP5(JL,JK) &
            & +(PLUDE5(JL,JK)+ZEVAPR5(JL,JK)+ZEVAPS5(JL,JK))*ZGDP (JL,JK)
    ZDQDT5(JL,JK)=-(ZCONDL5(JL,JK)+ZCONDI5(JL,JK)) &
            & +(PLUDE5(JL,JK)+ZEVAPR5(JL,JK)+ZEVAPS5(JL,JK))*ZGDP5(JL,JK)

    ZDTDT(JL,JK)= ZLVDCP (JL,JK)*ZCONDL5(JL,JK)+ZLSDCP (JL,JK)*ZCONDI5(JL,JK) &
            ...
            & +  PLUDE (JL,JK)*(ZFWAT5(JL,JK)*ZLVDCP5(JL,JK)+ &
            &        (1.0_JPRB-ZFWAT5(JL,JK))*ZLSDCP5(JL,JK))&
            & +  PLUDE5(JL,JK)*(ZFWAT (JL,JK)*(ZLVDCP5(JL,JK)-ZLSDCP5(JL,JK)) &
            & +                (ZFWAT5(JL,JK)*ZLVDCP (JL,JK)+ &
            &        (1.0_JPRB-ZFWAT5(JL,JK))*ZLSDCP (JL,JK))) &
            ...
            & *ZGDP (JL,JK)
    ZDTDT5(JL,JK)= ZLVDCP5(JL,JK)*ZCONDL5(JL,JK)+ZLSDCP5(JL,JK)*ZCONDI5(JL,JK) &
            & -( ZLVDCP5(JL,JK)*ZEVAPR5(JL,JK)+ZLSDCP5(JL,JK)*ZEVAPS5(JL,JK) &
            & +PLUDE5(JL,JK)*(ZFWAT5(JL,JK)*ZLVDCP5(JL,JK)+ &
            & (1.0_JPRB-ZFWAT5(JL,JK))*ZLSDCP5(JL,JK))&
            & -(ZLSDCP5(JL,JK)-ZLVDCP5(JL,JK))*ZRFREEZE5(JL,JK) )&
...
! first guess T and Q
    ZTP1 (JL,JK) = ZTP1 (JL,JK) + PTSPHY*ZDTDT (JL,JK)
    ZTP15(JL,JK) = ZTP15(JL,JK) + PTSPHY*ZDTDT5(JL,JK)
    ZQP1 (JL,JK) = ZQP1 (JL,JK) + PTSPHY*ZDQDT (JL,JK)
    ZQP15(JL,JK) = ZQP15(JL,JK) + PTSPHY*ZDQDT5(JL,JK)

    ZPP (JL,JK) = PAPP1 (JL,JK)
    ZPP5(JL,JK) = PAPP15(JL,JK)
    ZQOLD (JL,JK) = ZQP1 (JL,JK)
...
DO JL=KIDIA,KFDIA
  IF (ZTP15(JL,JK) > RTT) THEN
    Z3ES(JL,JK)=R3LES
    Z4ES(JL,JK)=R4LES
    Z5ALCP(JL,JK)=R5ALVCP
    ZALDCP(JL,JK)=RALVDCP
  ELSE
    Z3ES(JL,JK)=R3IES
    Z4ES(JL,JK)=R4IES
    Z5ALCP(JL,JK)=R5ALSCP
    ZALDCP(JL,JK)=RALSDCP
  ENDIF
ENDDO


  DO JL=KIDIA,KFDIA
    ZQP (JL,JK)=-ZPP(JL,JK)/ZPP5(JL,JK)**2
    ZQP5(JL,JK)=1.0_JPRB/ZPP5(JL,JK)
    ZTARG(JL,JK)=ZTP1(JL,JK)
    ZTARG5(JL,JK)=ZTP15(JL,JK)
    ZFOEEW5(JL,JK)=R2ES*EXP(Z3ES(JL,JK)*(ZTARG5(JL,JK)-RTT)/(ZTARG5(JL,JK)-Z4ES(JL,JK)))
    ZFOEEW (JL,JK)=Z3ES(JL,JK)*(RTT-Z4ES(JL,JK))*ZTARG(JL,JK)*ZFOEEW5(JL,JK)/&
     & (ZTARG5(JL,JK)-Z4ES(JL,JK))**2  
    ZQSAT (JL,JK)=ZQP5(JL,JK)*ZFOEEW (JL,JK) + ZQP (JL,JK)*ZFOEEW5(JL,JK)
    ZQSAT5(JL,JK)=ZQP5(JL,JK)*ZFOEEW5(JL,JK)
    IF (ZQSAT5(JL,JK) > ZQMAX) THEN
      ZQSAT (JL,JK)=0.0_JPRB
      ZQSAT5(JL,JK)=ZQMAX
    ENDIF
    ZCOR (JL,JK)=(RETV*ZQSAT(JL,JK))/(1.0_JPRB-RETV*ZQSAT5(JL,JK))**2
    ZCOR5(JL,JK)=1.0_JPRB/(1.0_JPRB-RETV  *ZQSAT5(JL,JK))
    ZQSAT (JL,JK)=ZQSAT5(JL,JK)*ZCOR (JL,JK) + ZQSAT (JL,JK)*ZCOR5(JL,JK)
    ZQSAT5(JL,JK)=ZQSAT5(JL,JK)*ZCOR5(JL,JK)
    Z2S (JL,JK)=-2.0_JPRB*ZTARG(JL,JK)*Z5ALCP(JL,JK)/(ZTARG5(JL,JK)-Z4ES(JL,JK))**3
    Z2S5(JL,JK)=    Z5ALCP(JL,JK)/(ZTARG5(JL,JK)-Z4ES(JL,JK))**2
    ZCOND1(JL,JK)=(ZQP1(JL,JK)-ZQSAT (JL,JK))/&
     & (1.0_JPRB+ZQSAT5(JL,JK)*ZCOR5(JL,JK)*Z2S5(JL,JK)) -&
     & (ZQP15(JL,JK)-ZQSAT5(JL,JK))*&
     & (ZQSAT (JL,JK)*ZCOR5(JL,JK)*Z2S5(JL,JK)   +&
     & ZQSAT5(JL,JK)*ZCOR (JL,JK)*Z2S5(JL,JK)   +&
     & ZQSAT5(JL,JK)*ZCOR5(JL,JK)*Z2S (JL,JK))/&
     & (1.0_JPRB+ZQSAT5(JL,JK)*ZCOR5(JL,JK)*Z2S5(JL,JK))**2  
    ZCOND15(JL,JK)=(ZQP15(JL,JK)-ZQSAT5(JL,JK))/(1.0_JPRB+ZQSAT5(JL,JK)*ZCOR5(JL,JK)*Z2S5(JL,JK))
    ZTP1(JL,JK)=ZTP1 (JL,JK)+ZALDCP(JL,JK)*ZCOND1(JL,JK)
    ZTP15(JL,JK)=ZTP15(JL,JK)+ZALDCP(JL,JK)*ZCOND15(JL,JK)
    ZQP1(JL,JK)=ZQP1(JL,JK)-ZCOND1(JL,JK)
    ZQP15(JL,JK)=ZQP15(JL,JK)-ZCOND15(JL,JK)
  ENDDO
...
!End of inlined cuadjtqstl

  DO JL=KIDIA,KFDIA
    IF ((ZQOLD5(JL,JK)-ZQP15(JL,JK)) >= 0.0_JPRB) THEN
      ZDQ5(JL,JK) = ZQOLD5(JL,JK)-ZQP15(JL,JK) 
      ZDQ (JL,JK) = ZQOLD (JL,JK)-ZQP1 (JL,JK)
! Regularization
      IF (LREGCL) THEN
        ZDQ (JL,JK) = ZDQ (JL,JK) * 0.7_JPRB
      ENDIF
    ELSE
      ZDQ (JL,JK) = 0.0_JPRB
      ZDQ5(JL,JK) = 0.0_JPRB
    ENDIF
    ZDR2  = ZCONS2*(ZDP5(JL,JK)*ZDQ(JL,JK)+ZDQ5(JL,JK)*ZDP(JL,JK))
    ZDR25 = ZCONS2*ZDP5(JL,JK)*ZDQ5(JL,JK)

! Update rain fraction and freezing.
! Note: impact of new temperature ZTP1 on ZFWAT is neglected here.
    IF (ZTP15(JL,JK) < RTT) THEN
      ZRFREEZE25=ZFWAT5(JL,JK)*ZDR25
      ZRFREEZE2 =ZFWAT (JL,JK)*ZDR25 + ZFWAT5(JL,JK)*ZDR2 
      ZFWATR5=0.0_JPRB
      ZFWATR =0.0_JPRB
    ELSE
      ZRFREEZE25=0.0_JPRB
...
! Note: The extra condensation due to the adjustment goes directly to precipitation
    ZCONDL (JL,JK)=ZCONDL (JL,JK)+(ZFWATR5*ZDQ (JL,JK)+ZFWATR *ZDQ5(JL,JK))*ZQTMST
    ZCONDL5(JL,JK)=ZCONDL5(JL,JK)+ ZFWATR5*ZDQ5(JL,JK)*ZQTMST

    ZCONDI (JL,JK)=ZCONDI (JL,JK)+((1.0_JPRB-ZFWATR5)*ZDQ (JL,JK)-ZFWATR *ZDQ5(JL,JK))*ZQTMST
    ZCONDI5(JL,JK)=ZCONDI5(JL,JK)+ (1.0_JPRB-ZFWATR5)*ZDQ5(JL,JK)*ZQTMST
  ...
  DO JL=KIDIA,KFDIA
    ...
    ZDLDT(JL,JK)=(ZQLWC (JL,JK)-ZL (JL,JK))*ZQTMST
    ZDLDT5(JL,JK)=(ZQLWC5(JL,JK)-ZL5(JL,JK))*ZQTMST

    ZDIDT(JL,JK)=(ZQIWC (JL,JK)-ZI (JL,JK))*ZQTMST
    ZDIDT5(JL,JK)=(ZQIWC5(JL,JK)-ZI5(JL,JK))*ZQTMST

    PTENQ (JL,JK) = ZDQDT (JL,JK)
    PTENQ5(JL,JK) = ZDQDT5(JL,JK)
    PTENT (JL,JK) = ZDTDT (JL,JK) 
    PTENT5(JL,JK) = ZDTDT5(JL,JK)
    PTENL (JL,JK) = ZDLDT(JL,JK)
    PTENL5(JL,JK) = ZDLDT5(JL,JK)
    PTENI (JL,JK) = ZDIDT(JL,JK) 
    PTENI5(JL,JK) = ZDIDT5(JL,JK)

    PFPLSL (JL,JK+1) = ZRFLN (JL,JK)
    PFPLSL5(JL,JK+1) = ZRFLN5(JL,JK)
    PFPLSN (JL,JK+1) = ZSFLN (JL,JK)
    PFPLSN5(JL,JK+1) = ZSFLN5(JL,JK)
  ENDDO

  DO JL=KIDIA,KFDIA
    ZRFL (JL,JK) = ZRFLN (JL,JK)
    ZRFL5(JL,JK) = ZRFLN5(JL,JK)
    ZSFL (JL,JK) = ZSFLN (JL,JK)
    ZSFL5(JL,JK) = ZSFLN5(JL,JK)
  ENDDO
