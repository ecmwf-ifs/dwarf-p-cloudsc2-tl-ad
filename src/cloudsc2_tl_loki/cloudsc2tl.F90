...
SUBROUTINE CLOUDSC2TL ( &
 ...
 & ZTP1SPHY, CETA,&
 & PAPHP15 , PAPP15  , ZQP1M15   , ZQP1S5   , ZTP1M15,  PL5 ,  PI5,&
 & PLUDE5  , PLU5    , PMFU5   , PMFD5,&
 & ZTP1ENT5  , PGTENT5 , ZTP1ENQ5  , PGTENQ5 ,&
 & ZTP1ENL5  , PGTENL5 , ZTP1ENI5  , PGTENI5 , PSUPSAT5, &
 & PCLC5   , PFPLSL5 , PFPLSN5 ,&
 & PFHPSL5 , PFHPSN5 , PCOVZTP1OT5,&
 & PAPHP1 , PAPP1    , ZQP1M1   , ZQP1S   , ZTP1M1,  PL ,  PI,&
 & PLUDE  , PLU      , PMFU   , PMFD,&
 & ZTP1ENT  , PGTENT   , ZTP1ENQ  , PGTENQ ,&
 & ZTP1ENL  , PGTENL   , ZTP1ENI  , PGTENI , PSUPSAT, &
 & PCLC   , PFPLSL   , PFPLSN , &
 & PFHPSL , PFHPSN   , PCOVZTP1OT, &
...
!    *ZTP1SPHY*       TIME STEP FOR THE PHYSICS                     S
!    *ZQP1M15*        SPECIFIC HUMIDITY AT T-1                  (Trajectory)
!    *ZQP1S5*         SATURATION SPECIFIC HUMIDITY              (Trajectory)
!    *ZTP1M15*        TEMPERATURE AT T-1                        (Trajectory)
...
!    *ZQP1M1*         SPECIFIC HUMIDITY AT T-1                     KG/KG
!    *ZQP1S*          SATURATION SPECIFIC HUMIDITY                 KG/KG
!    *ZTP1M1*         TEMPERATURE AT T-1                             K
...
!    *ZTP1ENT5*       TEMPERATURE TENDENCY                      (Trajectory)
!    *ZTP1ENQ5*       MOISTURE TENDENCY                         (Trajectory)
!    *ZTP1ENL5*       CLOUD LIQUID WATER TENDENCY               (Trajectory)
!    *ZTP1ENI5*       CLOUD ICE TENDENCY                        (Trajectory)
   
!    *ZTP1ENT*        TEMPERATURE TENDENCY                          K/S
!    *ZTP1ENQ*        MOISTURE TENDENCY                            KG/(KG S)
!    *ZTP1ENL*        CLOUD LIQUID WATER TENDENCY                  KG/(KG S)      
!    *ZTP1ENI*        CLOUD ICE  TENDENCY                          KG/(KG S)      
...
!    *PCOVZTP1OT5*    PRECIPITATION FRACTION IN EACH LAYER      (Trajectory) 
...
!    *PCOVZTP1OT*     PRECIPITATION FRACTION IN EACH LAYER
...
REAL(KIND=JPRB)   ,INTENT(IN)    :: ZTP1SPHY 
REAL(KIND=JPRB)   ,INTENT(IN)    :: PAPHP15(KLON,KLEV+1) 
REAL(KIND=JPRB)   ,INTENT(IN)    :: PAPP15(KLON,KLEV) 
REAL(KIND=JPRB)   ,INTENT(IN)    :: ZQP1M15(KLON,KLEV) 
REAL(KIND=JPRB)   ,INTENT(IN)    :: ZQP1S5(KLON,KLEV) 
REAL(KIND=JPRB)   ,INTENT(IN)    :: ZTP1M15(KLON,KLEV) 
...
REAL(KIND=JPRB)   ,INTENT(OUT)   :: ZTP1ENT5(KLON,KLEV) 
REAL(KIND=JPRB)   ,INTENT(OUT)   :: ZTP1ENQ5(KLON,KLEV) 
REAL(KIND=JPRB)   ,INTENT(OUT)   :: ZTP1ENL5(KLON,KLEV) 
REAL(KIND=JPRB)   ,INTENT(OUT)   :: ZTP1ENI5(KLON,KLEV) 
...
REAL(KIND=JPRB)   ,INTENT(OUT)   :: PCOVZTP1OT5(KLON,KLEV)
REAL(KIND=JPRB)   ,INTENT(IN)    :: PAPHP1(KLON,KLEV+1) 
REAL(KIND=JPRB)   ,INTENT(IN)    :: PAPP1(KLON,KLEV) 
REAL(KIND=JPRB)   ,INTENT(IN)    :: ZQP1M1(KLON,KLEV) 
REAL(KIND=JPRB)   ,INTENT(IN)    :: ZQP1S(KLON,KLEV) 
REAL(KIND=JPRB)   ,INTENT(IN)    :: ZTP1M1(KLON,KLEV) 
...
REAL(KIND=JPRB)   ,INTENT(OUT)   :: ZTP1ENT(KLON,KLEV) 
REAL(KIND=JPRB)   ,INTENT(OUT)   :: ZTP1ENQ(KLON,KLEV) 
REAL(KIND=JPRB)   ,INTENT(OUT)   :: ZTP1ENL(KLON,KLEV) 
REAL(KIND=JPRB)   ,INTENT(OUT)   :: ZTP1ENI(KLON,KLEV) 
...
REAL(KIND=JPRB)   ,INTENT(OUT)   :: PCOVZTP1OT(KLON,KLEV)
...
REAL(KIND=JPRB) :: ZQCRIT5(KLON), ZCOVZTP1OT5(KLON), ZCOVPCLR5(KLON)
...
REAL(KIND=JPRB) :: ZQSAT5(KLON), ZFWAT5(KLON)
...
REAL(KIND=JPRB) :: ZQCRIT(KLON), ZCOVZTP1OT(KLON), ZCOVPCLR(KLON)
...
REAL(KIND=JPRB) :: ZQP5(KLON), ZCOR5(KLON)
REAL(KIND=JPRB) :: ZQP(KLON), ZCOR(KLON)
...
ZCKCODTL=2.0_JPRB*RKCONV*ZTP1SPHY
ZCKCODTI=5.0_JPRB*RKCONV*ZTP1SPHY
ZCKCODTLA=ZCKCODTL/100._JPRB
ZCKCODTIA=ZCKCODTI/100._JPRB
ZCONS2 =1.0_JPRB/(ZTP1SPHY*RG)
ZCONS3 =RLVTT/RCPD
ZMELTP2=RTT+2.0_JPRB
ZQTMST=1.0_JPRB/ZTP1SPHY
...
DO JK=1,KLEV
  DO JL=KIDIA,KFDIA
    ZTP1 (JL,JK) = ZTP1M1 (JL,JK) + ZTP1SPHY*PGTENT (JL,JK)
    ZTP15(JL,JK) = ZTP1M15(JL,JK) + ZTP1SPHY*PGTENT5(JL,JK)
    ZQP1 (JL,JK) = ZQP1M1 (JL,JK) + ZTP1SPHY*PGTENQ (JL,JK) + PSUPSAT (JL,JK)
    ZQP15(JL,JK) = ZQP1M15(JL,JK) + ZTP1SPHY*PGTENQ5(JL,JK) + PSUPSAT5(JL,JK)
    ZL   (JL,JK) = PL   (JL,JK) + ZTP1SPHY*PGTENL (JL,JK)
    ZL5  (JL,JK) = PL5  (JL,JK) + ZTP1SPHY*PGTENL5(JL,JK)
    ZI   (JL,JK) = PI   (JL,JK) + ZTP1SPHY*PGTENI (JL,JK)
    ZI5  (JL,JK) = PI5  (JL,JK) + ZTP1SPHY*PGTENI5(JL,JK)
  ENDDO
ENDDO
...
DO JK=1,KLEV
  DO JL=KIDIA,KFDIA
    ...
    PCOVZTP1OT5(JL,JK) = 0.0_JPRB
    PCOVZTP1OT (JL,JK) = 0.0_JPRB
  ENDDO
ENDDO
...
DO JL=KIDIA,KFDIA
  ...
  ZCOVZTP1OT (JL) = 0.0_JPRB
  ZCOVZTP1OT5(JL) = 0.0_JPRB
...
!-----------------------------------
    ...
    ELSE
      ...
      Z4ES(JL)=R4LES
    ENDIF
    ZFOEEW5(JL) = R2ES*EXP(Z3ES(JL)*(ZTP15(JL,JK)-RTT)/(ZTP15(JL,JK)-Z4ES(JL)))
    ZFOEEW (JL) = Z3ES(JL)*(RTT-Z4ES(JL))*ZTP1(JL,JK)*ZFOEEW5(JL) &
              & / (ZTP15(JL,JK)-Z4ES(JL))**2  
    ...
    ZCOR(JL)  = RETV*ZESDP/(1.0_JPRB-RETV*ZESDP5)**2
    ZCOR5(JL) = 1.0_JPRB/(1.0_JPRB-RETV*ZESDP5)
    ZDQSDTEMP (JL) = ZFAC5*ZCOR5(JL)*ZQP1S(JL,JK)+ZFAC5*ZQP1S5(JL,JK)*ZCOR(JL) &
     & + ZCOR5(JL)*ZQP1S5(JL,JK)*ZFAC  
    ZDQSDTEMP5(JL) = ZFAC5*ZCOR5(JL)*ZQP1S5(JL,JK)
    ZCORQS (JL) = ZCONS3*ZDQSDTEMP(JL)
    ZCORQS5(JL) = 1.0_JPRB+ZCONS3*ZDQSDTEMP5(JL)

! use clipped state

    IF (ZQP15(JL,JK)>ZQP1S5(JL,JK)) THEN
      ZQLIM (JL) = ZQP1S (JL,JK)
      ZQLIM5(JL) = ZQP1S5(JL,JK)
    ELSE
      ZQLIM (JL) = ZQP1(JL,JK)
...
! Allow ice supersaturation at cold temperatures
    ...
    ELSE
      ...
      ZSUPSAT  = 0.0_JPRB
    ENDIF
    ZQSAT5(JL)=ZQP1S5(JL,JK)*ZSUPSAT5
    ZQSAT (JL)=ZQP1S (JL,JK)*ZSUPSAT5 + ZQP1S5(JL,JK)*ZSUPSAT
...
! Add convective component
  ...
  DO JL=KIDIA,KFDIA
    ...
    ZLUDE (JL,JK) = ZTP1SPHY*ZGDP5(JL)*PLUDE(JL,JK) &
     & + ZTP1SPHY*PLUDE5(JL,JK)*ZGDP(JL)  
    ZLUDE5(JL,JK) = PLUDE5(JL,JK)*ZTP1SPHY*ZGDP5(JL)
...
! Add compensating subsidence component
  ...
  DO JL=KIDIA,KFDIA
    ...
    ZRODQSDP = (-ZRHO *ZQP1S5(JL,JK) - ZRHO5*ZQP1S (JL,JK) + ZRHO5*ZQP1S5(JL,JK) &
           & *(PAPP1 (JL,JK)-RETV*ZFOEEW (JL))*ZFAC2)*ZFAC2
    ZRODQSDP5 = -ZRHO5*ZQP1S5(JL,JK)*ZFAC2
    ...
    LLO3=(ZDQSDZ5*(PMFU5(JL,JK)+PMFD5(JL,JK))*ZTP1SPHY*ZFAC4 < ZQC5(JL,JK))
    IF (LLO3) THEN
      ZDQC5 = ZDQSDZ5*(PMFU5(JL,JK)+PMFD5(JL,JK))*ZTP1SPHY*ZFAC4
      ZDQC  = ( ZTP1SPHY*(ZDQSDZ *(PMFU5(JL,JK)+PMFD5(JL,JK))&
...
! Simple form based on Maximum Overlap.
  ...
  DO JL=KIDIA,KFDIA  
    IF (PCLC5(JL,JK) > ZCOVZTP1OT5(JL)) THEN ! total rain frac
      ZCOVZTP1OT (JL) = PCLC (JL,JK)
      ZCOVZTP1OT5(JL) = PCLC5(JL,JK)
    ENDIF
    ZCOVPCLR (JL) = ZCOVZTP1OT(JL)-PCLC(JL,JK)
    ZCOVPCLR5(JL) = ZCOVZTP1OT5(JL)-PCLC5(JL,JK)
...
!   Precip evaporation
    ...
    IF (LLO2) THEN
      ZPRECLR  = (ZPRTOT5*ZCOVPCLR(JL)+ZCOVPCLR5(JL)*ZPRTOT) &
       & / ZCOVZTP1OT5(JL) &
       & - ZPRTOT5*ZCOVPCLR5(JL)*ZCOVZTP1OT(JL) &
       & / ZCOVZTP1OT5(JL)**2  
      ZPRECLR5 = ZPRTOT5*ZCOVPCLR5(JL)/ZCOVZTP1OT5(JL)

!     This is the humidity in the moistest zcovpclr region

      ZQE  = ZQP1S(JL,JK) - ((ZQP1S5(JL,JK)-ZQLIM5(JL))*ZCOVPCLR(JL) &
       & + ZCOVPCLR5(JL)*ZQP1S(JL,JK)-ZCOVPCLR5(JL)*ZQLIM(JL)) &
       & / (1.0_JPRB-PCLC5(JL,JK))**2 &
       & - 2.0_JPRB*(ZQP1S5(JL,JK)-ZQLIM5(JL))*ZCOVPCLR5(JL) &
       & * PCLC(JL,JK)/(1.0_JPRB-PCLC5(JL,JK))**3  
      ZQE5 = ZQP1S5(JL,JK)-(ZQP1S5(JL,JK)-ZQLIM5(JL))*ZCOVPCLR5(JL)&
       & / (1.0_JPRB-PCLC5(JL,JK))**2  
...
!     implicit solution:
      ...
      ZB  = ZTP1SPHY*((ZQP1S5(JL,JK)-ZQE5)*ZBETA+ZBETA5*ZQP1S(JL,JK) &
       & - ZBETA5*ZQE)/(1.0_JPRB+ZBETA5*ZTP1SPHY*ZCORQS5(JL)) &
       & - (ZTP1SPHY**2)*ZBETA5*(ZQP1S5(JL,JK)-ZQE5) &
       & * (ZBETA5*ZCORQS(JL)+ZCORQS5(JL)*ZBETA) &
       & / (1.0_JPRB+ZBETA5*ZTP1SPHY*ZCORQS5(JL))**2  
      ZB5 = ZTP1SPHY*ZBETA5*(ZQP1S5(JL,JK)-ZQE5) &
       & / (1.0_JPRB+ZBETA5*ZTP1SPHY*ZCORQS5(JL))  

!     exact solution:

!     ZEXP5 = EXP(-ZBETA5*ZCORQS5(JL)*ZTP1SPHY)
!     ZB  = (ZEXP5*(-ZCORQS5(JL)*(ZQP1S(JL,JK)-ZQE) &
!      &  + (ZQP1S5(JL,JK)-ZQE5) &
!      &  * (ZTP1SPHY*ZCORQS5(JL)*ZBETA5*ZCORQS(JL) &
!      &  + ZTP1SPHY*ZCORQS5(JL)*ZCORQS5(JL)*ZBETA + ZCORQS(JL))) &
!      &  + ZCORQS5(JL)*(ZQP1S(JL,JK)-ZQE) &
!      &  - (ZQP1S5(JL,JK)-ZQE5)*ZCORQS(JL)) &
!      &  / ZCORQS5(JL)**2
!     ZB5 =(ZQP1S5(JL,JK)-ZQE5)*(_ONE_-ZEXP5)/ZCORQS5(JL)

      ZDTGDP (JL) = - ZTP1SPHY*RG*(PAPHP1(JL,JK+1)-PAPHP1(JL,JK)) &
       & / (PAPHP15(JL,JK+1)-PAPHP15(JL,JK))**2  
      ZDTGDP5(JL) =ZTP1SPHY*RG/(PAPHP15(JL,JK+1)-PAPHP15(JL,JK))
      ...
      IF (ZPRECLR5 <= 0.0_JPRB) THEN !reset
        ...
        ZCOVZTP1OT5(JL) = PCLC5(JL,JK)
      ENDIF
      PCOVZTP1OT (JL,JK) = ZCOVZTP1OT (JL)
      PCOVZTP1OT5(JL,JK) = ZCOVZTP1OT5(JL)
...
! first guess T and Q
    ZTP1 (JL,JK) = ZTP1 (JL,JK) + ZTP1SPHY*ZDTDT (JL)
    ZTP15(JL,JK) = ZTP15(JL,JK) + ZTP1SPHY*ZDTDT5(JL)
    ZQP1 (JL,JK) = ZQP1 (JL,JK) + ZTP1SPHY*ZDQDT (JL)
    ZQP15(JL,JK) = ZQP15(JL,JK) + ZTP1SPHY*ZDQDT5(JL)
...
!OCL NOVREC
  DO JL=KIDIA,KFDIA
    ...
    ZCOND1 (JL)=(ZQP1 (JL,IK)-ZQSAT (JL))/&
     & (1.0_JPRB+ZQSAT5(JL)*ZCOR5(JL)*Z2S5(JL)) -&
     & (ZQP15(JL,IK)-ZQSAT5(JL))*&
     & (ZQSAT (JL)*ZCOR5(JL)*Z2S5(JL)   +&
     & ZQSAT5(JL)*ZCOR (JL)*Z2S5(JL)   +&
     & ZQSAT5(JL)*ZCOR5(JL)*Z2S (JL))/&
     & (1.0_JPRB+ZQSAT5(JL)*ZCOR5(JL)*Z2S5(JL))**2  
    ZCOND15(JL)=(ZQP15(JL,IK)-ZQSAT5(JL))/(1.0_JPRB+ZQSAT5(JL)*ZCOR5(JL)*Z2S5(JL))
    ZTP1 (JL,IK)=ZTP1 (JL,IK)+ZALDCP(JL)*ZCOND1 (JL)
    ZTP15(JL,IK)=ZTP15(JL,IK)+ZALDCP(JL)*ZCOND15(JL)
    ZQP1 (JL,IK)=ZQP1 (JL,IK)-ZCOND1 (JL)
    ZQP15(JL,IK)=ZQP15(JL,IK)-ZCOND15(JL)
  ENDDO
...
! Note: The extra condensation due to the adjustment goes directly to precipitation
  ...
  DO JL=KIDIA,KFDIA
    ...
    ZTP1ENQ (JL,JK) = ZDQDT (JL)
    ZTP1ENQ5(JL,JK) = ZDQDT5(JL)
    ZTP1ENT (JL,JK) = ZDTDT (JL) 
    ZTP1ENT5(JL,JK) = ZDTDT5(JL)
    ZTP1ENL (JL,JK) = ZDLDT (JL)
    ZTP1ENL5(JL,JK) = ZDLDT5(JL)
    ZTP1ENI (JL,JK) = ZDIDT (JL) 
    ZTP1ENI5(JL,JK) = ZDIDT5(JL)
