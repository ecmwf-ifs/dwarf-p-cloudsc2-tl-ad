...
@click.command()
@click.option(
    "--num-threads", type=int, default=1, help="Number of OpenMP threads."
)
def main(num_threads: int = 1) -> None:
    ...
    for mode, nx in itertools.product(mode_l, nx_l):
        ...
        for backend in backend_l:
            ...
            run_python(mode, backend, nx, nz, nruns=3, csv_file=None)

            for i in range(nruns):
                # python: run and profile
                run_python(mode, backend, nx, nz, nruns=3, csv_file=csv_file)
                print(
