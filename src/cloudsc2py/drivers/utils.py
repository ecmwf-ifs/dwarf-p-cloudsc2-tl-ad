...
def log_performance(
    nruns: int,
    timer: Type["Timer"],
    exec_info: Dict[str, Dict[str, Union[bool, float]]],
    ...
    if nruns > 0:
        ...
        print(
            f"\nAverage run time ({nruns} runs): {total_time / nruns:.3f} ms\n"
            f"  - GT4Py (stencil calculations):"
            f" {(cpp_time if cpp_time > 0 else call_time) / nruns:.3f} ms\n"
            f"  - GT4Py (bindings overhead):"
            f" {(call_time - cpp_time) / nruns:.3f} ms\n"
