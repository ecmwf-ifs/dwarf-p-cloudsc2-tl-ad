...
@function_collection("cloudsc2_ad_1")
@gtscript.function
def cloudsc2_ad_1(
    in_ap,
    in_aph,
    ...
    lsdcp_i += dtdt_i * (condi1 - gdp * (evaps + (1 - fwat) * in_lude - rfreeze1))
    out_lude_i += -dtdt_i * gdp * (fwat * lvdcp + (1 - fwat) * lsdcp) + dqdt_i * gdp
    fwat_i -= dtdt_i * in_lude * gdp * (lvdcp - lsdcp)
    rfreeze_i += dtdt_i * (lsdcp - lvdcp) * gdp

    if prtot > ext.ZEPS2 and covpclr > ext.ZEPS2 and (ext.LEVAPLS2 or ext.LDRAIN1D):
        # ice proportion
        evaps_i -= sfln_i
        ...
        out_ap_i += 0.5 * xx * preclr1 * beta_i / (covpclr * sqrt(in_ap * aph_s))
        aphn_i_s -= 0.5 * xx * preclr1 * sqrt(in_ap / aph_s) * beta_i / (covpclr * aph_s)
        covpclr_i -= (
    ...
    out_aph_i += ext.RG * gdp_i[0, 0, 0] / (in_aph[0, 0, 1] - in_aph[0, 0, 0]) ** 2
    daph_i += ext.RG * gdp_i[0, 0, 0] / (in_aph[0, 0, 1] - in_aph[0, 0, 0]) ** 2
    ...
    if qt < qcrit:
        ...
        qt_i = (-0.5 / tmp3 * (qpd * scalm * clc_i) / (qcd - scalm * (qt - qcrit)) ** 2) - qpd_i
        qcrit_i = (0.5 / tmp3 * (qpd * scalm * clc_i) / (qcd - scalm * (qt - qcrit)) ** 2) - qcd_i
        qsat_i = qcd_i + qpd_i
    ...
    out_t_i -= 2 * (
        ext.R5IES * faci_i / (t2 - ext.R4IES) ** 3 + ext.R5LES * facw_i / (t2 - ext.R4LES) ** 3
    )
