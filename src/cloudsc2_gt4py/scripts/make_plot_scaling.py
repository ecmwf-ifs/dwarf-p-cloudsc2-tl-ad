...
def get_csv_file(nx: int, mode: str) -> str:
    return f"20220901_dom_{mode}_{nx}_137_24.csv"
...
line_pool_nl = LinePool(partial(get_csv_file, mode="nl"), "$\\mathbf{(a)}$ CLOUDSC2-NL", lines)
line_pool_tl = LinePool(
    partial(get_csv_file, mode="tl"), "$\\mathbf{(b)}$ CLOUDSC2-TL", lines[0:1] + lines[2:]
)
line_pool_ad = LinePool(
    partial(get_csv_file, mode="ad"), "$\\mathbf{(c)}$ CLOUDSC2-AD", lines[0:1] + lines[2:]
)
    ...
    for nx in nx_l:
        ...
        for line in line_pool.lines:
            if line.column in df.columns:
                ...
                if line.column == "dace:gpu" and runtime > reference_runtime:
