...
from cloudsc2py.framework.stencil import function_collection, stencil_collection
...
@function_collection("cloudsc2_nl_fvm_0")
@gtscript.function
def cloudsc2_nl_fvm_0(
    in_eta,
    in_ap,
    in_aph,
    in_q,
    ...
    rh1 = 1.0
    rh2 = 0.35 + 0.14 * ((trpaus - 0.25) / 0.15) ** 2 + 0.04 * min(trpaus - 0.25, 0.0) / 0.15
    rh3 = 1.0
    if in_eta < trpaus:
        crh2 = rh3
    else:
        ...
        bound1 = trpaus + deta2
        if in_eta < bound1:
            crh2 = rh3 + (rh2 - rh3) * (in_eta - trpaus) / deta2
        else:
            deta1 = 0.09 + 0.16 * (0.4 - trpaus) / 0.3
            bound2 = 1 - deta1
            if in_eta < bound2:
                crh2 = rh2
            else:
    ...
    if prtot > ZEPS2 and covpclr > ZEPS2 and (LEVAPLS2 or LDRAIN1D):
        ...
        beta = (
            RG
            * RPECONS
            * (sqrt(in_ap[0, 0, 0] / in_aph[0, 0, 1]) / 0.00509 * preclr / covpclr) ** 0.5777
        )
...
@stencil_collection("cloudsc2_nl_fvm")
def cloudsc2_nl_fvm_def(
    ...
    tmp_rfl: gtscript.Field["float"],
    tmp_sfl: gtscript.Field["float"],
    *,
    dt: "float",
):
    ...
    with computation(FORWARD):
        with interval(0, 1):
            flag = in_eta[0] > 0.1 and in_eta[0] < 0.4 and t[0, 0, 0] > t[0, 0, 1]
            trpaus = in_eta[0] if flag else 0.1
        with interval(1, -1):
            flag = in_eta[0] > 0.1 and in_eta[0] < 0.4 and t[0, 0, 0] > t[0, 0, 1]
            trpaus = in_eta[0] if flag else trpaus[0, 0, -1]
...
@stencil_collection("cloudsc2_nl_fvm_fluxes")
def cloudsc2_nl_fvm_fluxes_def(
    ...
    with computation(FORWARD):
        with interval(0, 1):
            ...
            out_fhpsn[0, 0, 0] = 0.0
        with interval(1, None):
            out_fplsl[0, 0, 0] = tmp_rfl[0, 0, -1]
            out_fplsn[0, 0, 0] = tmp_sfl[0, 0, -1]
