...
def cloudsc2_nl_def(
    ...
    *,
    ...
    with computation(FORWARD), interval(0, -1):
        ...
        if lo1:
            out_clc[0, 0, 0] += (1 - out_clc[0, 0, 0]) * (1 - exp(-lude[0, 0, 0] / in_lu[0, 0, 1]))
            qc += lude
        ...
        if prtot > ZEPS2 and covpclr > ZEPS2 and (LEVAPLS2 or LDRAIN1D):
            ...
            qe = in_qsat[0, 0, 0] - (in_qsat[0, 0, 0] - qlim) * covpclr / ((1 - out_clc[0, 0, 0]) ** 2)
            beta = RG * RPECONS * (sqrt(in_ap[0, 0, 0] / tmp_aph_s[0, 0]) / 0.00509 * preclr / covpclr) ** 0.5777
