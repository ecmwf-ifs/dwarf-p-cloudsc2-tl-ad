...
from src.cloudsc2_python.utils import ported_method
...
if TYPE_CHECKING:
    ...
    from src.cloudsc2_python.utils import ParameterDict, StorageDict
...
class Cloudsc2NL(ImplicitTendencyComponent):
    def __init__(
        ...
        *,
        ...
        self.cloudsc2 = self.compile_stencil("cloudsc2_nl_fvm", externals)
        self.cloudsc2_fluxes = self.compile_stencil("cloudsc2_nl_fvm_fluxes", externals)

        # # allocate temporary arrays
        # allocate_f = partial(
        #     get_array,
        #     self.grid,
        #     (self.grid.dims_x, self.grid.dims_y, self.grid.dims_z),
        #     backend=backend,
    ...
    def array_call(
        ...
        with managed_temporary_storage(
            self.computational_grid, *repeat((I, J, K), 2), gt4py_config=self.gt4py_config
        ) as (tmp_rfl, tmp_sfl):
            self.cloudsc2(
                in_eta=state["f_eta"],
                in_ap=state["f_ap"],
