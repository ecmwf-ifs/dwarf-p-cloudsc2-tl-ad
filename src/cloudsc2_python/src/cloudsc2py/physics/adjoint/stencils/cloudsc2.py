    ...
    *,
    ...
    with computation(FORWARD):
        ...
        with interval(1, -1):
            ...
            out_fplsl[0, 0, 0] = rfl[0, 0, -1]
            out_fplsn[0, 0, 0] = sfl[0, 0, -1]
    ...
    with computation(BACKWARD):
        with interval(-1, None):
            ...
            fplsn_i = in_fplsn_i[0, 0, 0] - in_fhpsn_i[0, 0, 0] * RLSTT
            fplsl_i = in_fplsl_i[0, 0, 0] - in_fhpsl_i[0, 0, 0] * RLVTT
        with interval(-2, -1):
            ...
            lsdcp_i = in_tnd_t_i[0, 0, 0] * (condi2 - gdp * (evaps + in_lude[0, 0, 0] * (1 - fwat) - rfreeze3))
            out_lude_i[0, 0, 0] = gdp * (-in_tnd_t_i[0, 0, 0] * (fwat * lvdcp + (1 - fwat) * lsdcp) + in_tnd_q_i[0, 0, 0])
            ...
            lsdcp_i -= dtdt_i * in_lude[0, 0, 0] * gdp * (1 - fwat)
            ...
            if prtot > ZEPS2 and covpclr > ZEPS2 and (LEVAPLS2 or LDRAIN1D):
                ...
                covptot_i = in_covptot_i[0, 0, 0] + 0
                if preclr <= 0:
                    clc_i = in_clc_i[0, 0, 0] + covptot_i
                    covptot_i = 0.0
                else:
                    clc_i = in_clc_i[0, 0, 0] + 0
                ...
                out_aph_i[0, 0, 0] = dt * RG * dtgdp_i / (in_aph[0, 0, 1] - in_aph[0, 0, 0])
                ...
                beta_i = (
                    ...
                    - (dt ** 2) * beta * (in_qsat[0, 0, 0] - qe) * corqs * b_i / tmp1 ** 2
                )
                out_qsat_i[0, 0, 0] = dt * beta * b_i / tmp1
                qe_i = -dt * beta * b_i / tmp1
                corqs_i = -(dt ** 2) * beta * (in_qsat[0, 0, 0] - qe) * beta * b_i / tmp1 ** 2

                # this is the humidity in the moistest covpclr region
                xx = (
                    0.5777
                    * (RG * RPECONS / 0.00509)
                    * (0.00509 * covpclr / (preclr1 * sqrt(in_ap[0, 0, 0] / tmp_aph_s[0, 0]))) ** 0.4223
                )
                preclr_i += xx * sqrt(in_ap[0, 0, 0] / tmp_aph_s[0, 0]) * beta_i / covpclr
                out_ap_i[0, 0, 0] += 0.5 * xx * preclr1 * beta_i / (covpclr * sqrt(in_ap[0, 0, 0] * tmp_aph_s[0, 0]))
                aph_i_s -= (
                    0.5 * xx * preclr1 * sqrt(in_ap[0, 0, 0] / tmp_aph_s[0, 0]) * beta_i / (covpclr * tmp_aph_s[0, 0])
                )
                covpclr_i -= (
                    (xx * preclr1 * sqrt(in_ap[0, 0, 0] / tmp_aph_s[0, 0]) * beta_i / covpclr ** 2)
                    + (in_qsat[0, 0, 0] - qlim) * qe_i / (1 - out_clc[0, 0, 0]) ** 2
                ) - prtot * preclr_i / covptot1
                out_qsat_i[0, 0, 0] += qe_i - covpclr * qe_i / (1 - out_clc[0, 0, 0]) ** 2
                qlim_i = covpclr * qe_i / (1 - out_clc[0, 0, 0]) ** 2
                clc_i -= 2 * (in_qsat[0, 0, 0] - qlim) * covpclr * qe_i / (1 - out_clc[0, 0, 0]) ** 3
            ...
            if out_clc[0, 0, 0] > ZEPS2:
                ...
                clc_i -= qiwc1 * cldi_i / out_clc[0, 0, 0] ** 2
                ...
                clc_i -= qlwc1 * cldl_i / out_clc[0, 0, 0] ** 2

            if sfl[0, 0, -1] != 0.0:
                snmlt_i = -out_t_i[0, 0, 0] / cons + rfl_i - sfl_i
                ...
                if t2 > meltp2:
                    cons_i = out_t_i[0, 0, 0] * snmlt / cons ** 2 + (t2 - meltp2) * z2s_i
                    out_t_i[0, 0, 0] += cons * z2s_i
                else:
                    cons_i = out_t_i[0, 0, 0] * snmlt / cons ** 2
            ...
            if lo3:
                if __INLINED(LREGCL):
                    # regularization
                    dqc_i *= 0.1
                dqsdz_i = dt * dqc_i * (in_mfd[0, 0, 0] + in_mfu[0, 0, 0]) / rho
                out_mfd_i[0, 0, 0] = dt * dqc_i * dqsdz / rho
                out_mfu_i[0, 0, 0] = dt * dqc_i * dqsdz / rho
                rho_i = -dqc_i * dqc / rho
            else:
                qc_i += dqc_i
                dqsdz_i = 0.0
            ...
            out_ap_i[0, 0, 0] += rodqsdp_i * rho * in_qsat[0, 0, 0] * fac2 ** 2
            foeew_i = -RETV * rodqsdp_i * rho * in_qsat[0, 0, 0] * fac2 ** 2
            out_ap_i[0, 0, 0] += rho_i * fac1
            ...
            out_aph_i[0, 0, 0] += RG * gdp_i[0, 0, 0] / (in_aph[0, 0, 1] - in_aph[0, 0, 0]) ** 2
            ...
            out_t_i[0, 0, 0] -= 2 * (R5IES * faci_i / (t2 - R4IES) ** 3 + R5LES * facw_i / (t2 - R4LES) ** 3)

            if esdp1 > ZQMAX:
                esdp_i = 0.0
            foeew_i += esdp_i / in_ap[0, 0, 0]
            out_ap_i[0, 0, 0] -= esdp_i * foeew / in_ap[0, 0, 0] ** 2
        ...
        with interval(1, -2):
            ...
            out_lude_i[0, 0, 0] = -in_tnd_t_i[0, 0, 0] * gdp * (fwat * lvdcp + (1 - fwat) * lsdcp)
            lvdcp_i -= in_tnd_t_i[0, 0, 0] * in_lude[0, 0, 0] * gdp * fwat
            lsdcp_i -= in_tnd_t_i[0, 0, 0] * in_lude[0, 0, 0] * gdp * (1 - fwat)
            fwat_i = -in_tnd_t_i[0, 0, 0] * in_lude[0, 0, 0] * gdp * (lvdcp - lsdcp)
            ...
            out_t_i[0, 0, 0] = 0.0
            out_ap_i[0, 0, 0], told, out_t_i[0, 0, 0], qold, out_q_i[0, 0, 0] = cuadjtqs_ad(
                in_ap, out_ap_i[0, 0, 0], told, out_t_i[0, 0, 0], qold, out_q_i[0, 0, 0]
            )
    ...
    with computation(BACKWARD):
        with interval(-1, None):
            out_aph_i[0, 0, 0] += dp_i[0, 0, -1] - daph_i[0, 0, -1]
        with interval(1, -1):
            out_aph_i[0, 0, 0] += -dp_i[0, 0, 0] + dp_i[0, 0, -1] - daph_i[0, 0, -1]
            out_lu_i[0, 0, 0] = -dlu_i
