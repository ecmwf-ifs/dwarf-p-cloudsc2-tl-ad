...
MODULE CLOUDSC_DRIVER_MOD
  ...
  SUBROUTINE CLOUDSC_DRIVER (NUMOMP, NPROMA, NLEV, NGPTOT, NGPTOTG, NGPBLKS, PTSPHY, PT, PQ, TENDENCY_CML, TENDENCY_LOC,  &
  & BUFFER_CML, BUFFER_LOC, PAP, PAPH, PLU, PLUDE, PMFU, PMFD, PA, PCLV, PSUPSAT, PCOVPTOT, PFPLSL, PFPLSN, PFHPSL, PFHPSN,  &
  & YDCST, YDTHF, YHNC, YPHLI, YCLD, YCLDP)
    ...
    USE CLOUDSC2_SCC_MOD, ONLY: CLOUDSC2_SCC
    USE SATUR_SCC_MOD, ONLY: SATUR_SCC
...
!!private( ZQSAT )
    DO JKGLO=1,NGPTOT,NPROMA
      ...
      ! Fill in ZQSAT
      CALL SATUR_SCC(1, ICEND, NPROMA, 1, NLEV, .true., PAP(:, :, IBL), PT(:, :, IBL), ZQSAT(:, :), 2, LOCAL_YDCST, LOCAL_YDTHF)
      print *,'Przed cloudsc'      
      CALL CLOUDSC2_SCC(1, ICEND, NPROMA, 1, NLEV, LDRAIN1D, PTSPHY, LOCAL_YCLD%CETA, PAPH(:, :, IBL), PAP(:, :, IBL), PQ(:, :, IBL),  &
      & ZQSAT(:, :), PT(:, :, IBL), PCLV(:, :, NCLDQL, IBL), PCLV(:, :, NCLDQI, IBL), PLUDE(:, :, IBL), PLU(:, :, IBL),  &
      & PMFU(:, :, IBL), PMFD(:, :, IBL), BUFFER_LOC(:, :, 1, IBL), BUFFER_CML(:, :, 1, IBL), BUFFER_LOC(:, :, 3, IBL),  &
      & BUFFER_CML(:, :, 3, IBL), BUFFER_LOC(:, :, 3 + NCLDQL, IBL), BUFFER_CML(:, :, 3 + NCLDQL, IBL),  &
      & BUFFER_LOC(:, :, 3 + NCLDQI, IBL), BUFFER_CML(:, :, 3 + NCLDQI, IBL), PSUPSAT(:, :, IBL), PA(:, :, IBL),  &
      & PFPLSL(:, :, IBL), PFPLSN(:, :, IBL), PFHPSL(:, :, IBL), PFHPSN(:, :, IBL), PCOVPTOT(:, :, IBL), LOCAL_YDCST,  &
      & LOCAL_YDTHF, LOCAL_YHNC, LOCAL_YPHLI, LOCAL_YCLD, LOCAL_YCLDP)
