...
!
MODULE CLOUDSC2_SCC_MOD
  
  CONTAINS
  
  SUBROUTINE CLOUDSC2_SCC (KIDIA, KFDIA, KLON, KTDIA, KLEV, LDRAIN1D, PTSPHY, CETA, PAPHP1, PAPP1, PQM1, PQS, PTM1, PL, PI,  &
  & PLUDE, PLU, PMFU, PMFD, PTENT, PGTENT, PTENQ, PGTENQ, PTENL, PGTENL, PTENI, PGTENI, PSUPSAT, PCLC, PFPLSL, PFPLSN, PFHPSL,  &
  & PFHPSN, PCOVPTOT, YDCST, YDTHF, YHNC, YPHLI, YCLD, YCLDP)
    ...
    !     ------------------------------------------------------------------
!$acc routine vector
!$acc data present( YCLD%CETA, PAPHP1, PAPP1, PQM1, PQS, PTM1, PL, PI, PLUDE, PLU, PMFU, PMFD, PTENT, PGTENT, PTENQ, PGTENQ, PTENL,  &
!$acc & PGTENL, PTENI, PGTENI, PSUPSAT, PCLC, PFPLSL, PFPLSN, PFHPSL, PFHPSN, PCOVPTOT, YDCST, YDTHF, YHNC, YPHLI, YCLD, YCLDP )
    
    print *, 'cloudssc start'
!$acc loop vector
    DO JL=KIDIA,KFDIA
    print *, 'cloudsc JL start:',JL,'of',KFDIA
      ...
      ZCKCODTL = 2.0_JPRB*YCLDP%RKCONV*PTSPHY
      ZCKCODTI = 5.0_JPRB*YCLDP%RKCONV*PTSPHY
      ZCONS2 = 1.0_JPRB / (PTSPHY*YDCST%RG)
      ZCONS3 = YDCST%RLVTT / YDCST%RCPD
      ZMELTP2 = YDCST%RTT + 2.0_JPRB
...
!$acc loop seq
    ...
    print *, 'cloudsc JL #2:',JL
!$acc loop seq
      DO JK=1,KLEV
        
        ! Parameter for cloud formation
        
         ZSCALM(JL, JK) = ZSCAL*MAX((YCLD%CETA(JK) - 0.2_JPRB), ZEPS1)**0.2_JPRB
        
        
        ! thermodynamic constants
        
        ZDP(JL, JK) = PAPHP1(JL, JK + 1) - PAPHP1(JL, JK)
        ZZZ = 1.0_JPRB / (YDCST%RCPD + YDCST%RCPD*YDTHF%RVTMP2*ZQP1(JL, JK))
        ZLFDCP(JL, JK) = YDCST%RLMLT*ZZZ
        ZLSDCP(JL, JK) = YDCST%RLSTT*ZZZ
        ZLVDCP(JL, JK) = YDCST%RLVTT*ZZZ
        LLFLAG(JL) = .true.
      END DO
...
!$acc loop seq
      ...
      ZTRPAUS(JL) = 0.1_JPRB
    print *, 'cloudsc JL #4:',JL
!$acc loop seq
      DO JK=1,KLEV - 1
        LLO1 = YCLD%CETA(JK) > 0.1_JPRB .and. YCLD%CETA(JK) < 0.4_JPRB .and. ZTP1(JL, JK) > ZTP1(JL, JK + 1)
        IF (LLO1) THEN
          ZTRPAUS(JL) = YCLD%CETA(JK)
        END IF
      END DO
...
!$acc loop seq
      DO JK=KTDIA,KLEV
        ...
        IF (YPHLI%LPHYLIN .or. LDRAIN1D) THEN
          ZOEALFAW = 0.545_JPRB*(TANH(0.17_JPRB*(ZTP1(JL, JK) - YPHLI%RLPTRC)) + 1.0_JPRB)
          ...
          ELSE
            ...
            Z4ES = YDTHF%R4LES
          END IF
          ZFOEEW(JL) = YDTHF%R2ES*EXP(Z3ES*(ZTP1(JL, JK) - YDCST%RTT) / (ZTP1(JL, JK) - Z4ES))
        ...
        ZRH1 = 1.0_JPRB
        ZRH2 =  &
        & 0.35_JPRB + 0.14_JPRB*((ZETA3 - 0.25_JPRB) / 0.15_JPRB)**2 + 0.04_JPRB*MIN(ZETA3 - 0.25_JPRB, 0.0_JPRB) / 0.15_JPRB
        ZRH3 = 1.0_JPRB
        ZDETA2 = 0.3_JPRB
        ZDETA1 = 0.09_JPRB + 0.16_JPRB*(0.4_JPRB - ZETA3) / 0.3_JPRB
        IF (YCLD%CETA(JK) < ZETA3) THEN
          ZCRH2 = ZRH3
        ELSE IF (YCLD%CETA(JK) >= ZETA3 .and. YCLD%CETA(JK) < (ZETA3 + ZDETA2)) THEN
          ZCRH2 = ZRH3 + (ZRH2 - ZRH3)*((YCLD%CETA(JK) - ZETA3) / ZDETA2)
        ELSE IF (YCLD%CETA(JK) >= (ZETA3 + ZDETA2) .and. YCLD%CETA(JK) < (1.0_JPRB - ZDETA1)) THEN
          ZCRH2 = ZRH2
        ELSE IF (YCLD%CETA(JK) >= (1.0_JPRB - ZDETA1)) THEN
          ZCRH2 = ZRH1 + (ZRH2 - ZRH1)*((1.0_JPRB - YCLD%CETA(JK)) / ZDETA1)**0.5_JPRB
        END IF
        ! Allow ice supersaturation at cold temperatures
        IF (ZTP1(JL, JK) < YDTHF%RTICE) THEN
        ...
        ELSE
          ...
          ZQC(JL, JK) = (ZSCALM(JL, JK)*ZQPD + (1.0_JPRB - ZSCALM(JL, JK))*ZQCD)*PCLC(JL, JK)**2
        END IF
        
        ! Add convective component
        
        ZGDP(JL) = YDCST%RG / (PAPHP1(JL, JK + 1) - PAPHP1(JL, JK))
        ZLUDE(JL, JK) = PLUDE(JL, JK)*PTSPHY*ZGDP(JL)
        IF (JK < KLEV) THEN
          LLO1 = ZLUDE(JL, JK) >= YCLDP%RLMIN .and. PLU(JL, JK + 1) >= ZEPS2
        ELSE
          LLO1 = .false.
        END IF
        IF (LLO1) THEN
          PCLC(JL, JK) = PCLC(JL, JK) + (1.0_JPRB - PCLC(JL, JK))*(1.0_JPRB - EXP(-ZLUDE(JL, JK) / PLU(JL, JK + 1)))
          ZQC(JL, JK) = ZQC(JL, JK) + ZLUDE(JL, JK)
        END IF
        
        ! Add compensating subsidence component
        
        ZFAC1 = 1.0_JPRB / (YDCST%RD*ZTP1(JL, JK))
        ZRHO = PAPP1(JL, JK)*ZFAC1
        ZFAC2 = 1.0_JPRB / (PAPP1(JL, JK) - YDCST%RETV*ZFOEEW(JL))
        ...
        DTDZMO = YDCST%RG*(1.0_JPRB / YDCST%RCPD - ZLDCP*ZRODQSDP)*ZFAC3
        ZDQSDZ = ZDQSDTEMP(JL)*DTDZMO - YDCST%RG*ZRODQSDP
        ZFAC4 = 1.0_JPRB / ZRHO
        ...
        IF (PCLC(JL, JK) > ZCOVPTOT(JL)) THEN
          ! total rain frac
        ...
        IF (PCLC(JL, JK) > ZEPS2) THEN
          IF (YHNC%LEVAPLS2 .or. LDRAIN1D) THEN
            ZLCRIT = 1.9_JPRB*YCLDP%RCLCRIT
          ELSE
            ZLCRIT = YCLDP%RCLCRIT*2._JPRB
          END IF
        ...
        IF (PCLC(JL, JK) > ZEPS2) THEN
          ...
          ZD = ZCKCODTI*EXP(0.025_JPRB*(ZTP1(JL, JK) - YDCST%RTT))*(1.0_JPRB - EXP(-(ZCLDI / ZLCRIT)**2))
        ...
        LLO2 = ZPRTOT > ZEPS2 .and. ZCOVPCLR(JL) > ZEPS2 .and. (YHNC%LEVAPLS2 .or. LDRAIN1D)
        IF (LLO2) THEN
          ...
          ZQE = PQS(JL, JK) - (PQS(JL, JK) - ZQLIM(JL))*ZCOVPCLR(JL) / (1.0_JPRB - PCLC(JL, JK))**2
          ZBETA = YDCST%RG*YCLDP%RPECONS*(SQRT(PAPP1(JL, JK) / PAPHP1(JL, KLEV + 1)) / 5.09E-3_JPRB*ZPRECLR / ZCOVPCLR(JL)) &
          & **0.5777_JPRB
        ...
        ZDQDT(JL) = -(ZCONDL(JL, JK) + ZCONDI(JL, JK)) + (PLUDE(JL, JK) + ZEVAPR(JL, JK) + ZEVAPS(JL, JK))*ZGDP(JL)
        
        ZDTDT(JL) = ZLVDCP(JL, JK)*ZCONDL(JL, JK) + ZLSDCP(JL, JK)*ZCONDI(JL, JK) - (ZLVDCP(JL, JK)*ZEVAPR(JL, JK) + ZLSDCP(JL,  &
        & JK)*ZEVAPS(JL, JK) + PLUDE(JL, JK)*(ZFWAT(JL)*ZLVDCP(JL, JK) + (1.0_JPRB - ZFWAT(JL))*ZLSDCP(JL, JK)) - (ZLSDCP(JL, JK) &
        &  - ZLVDCP(JL, JK))*ZRFREEZE(JL, JK))*ZGDP(JL)
        ...
        ZQP = 1.0_JPRB / ZPP(JL)
        ZTARG = ZTP1(JL, JK)
        ZFOEEW(JL) = YDTHF%R2ES*EXP(Z3ES*(ZTARG - YDCST%RTT) / (ZTARG - Z4ES))
        ZQSAT(JL) = ZQP*ZFOEEW(JL)
        IF (ZQSAT(JL) > ZQMAX) THEN
          ZQSAT(JL) = ZQMAX
        END IF
        ZCOR = 1.0_JPRB / (1.0_JPRB - YDCST%RETV*ZQSAT(JL))
    ...
    print *, 'cloudsc JL #6:',JL
!$acc loop seq
      DO JK=1,KLEV + 1
        PFHPSL(JL, JK) = -PFPLSL(JL, JK)*YDCST%RLVTT
        PFHPSN(JL, JK) = -PFPLSN(JL, JK)*YDCST%RLSTT
      END DO
    ...
    END DO
    print *, 'cloudsc JL end:',JL
!$acc end data
  END SUBROUTINE CLOUDSC2_SCC
  
END MODULE CLOUDSC2_SCC_MOD
